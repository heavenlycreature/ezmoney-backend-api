...
const saveUserWallet = async (req, res) => {
  ...
  try {
      ...
      // Add transaction to monthly records
     ...
     // Fetch all previous monthly summaries to get the latest balance
        const previousMonthsQuery = userCollection.doc(userId)
      ...
      if (!monthlySummaryDoc.exists) {
          ...
          updatedSummary = {
              ...
              saving: parsedSaving,
              recommendedSavings: type === 'income' 
                  ? Math.round(parsedAmount * (parsedSaving / 100)) 
                  : 0,
              lastUpdated: isoDate,
          ...
          const newTotalIncome = type === 'income' 
              ? currentSummary.totalIncome + parsedAmount 
              : currentSummary.totalIncome;
          
          const newTotalExpenses = type === 'expenses' 
              ? currentSummary.totalExpenses + parsedAmount 
